...
class Session(db.Model):
    ...
    speakers: Mapped[List["Speaker"]] = relationship(
        secondary=session_speaker, back_populates="sessions"
    )
    materials: Mapped[List["PresentationMaterial"]] = relationship(
    ...
    def remove_speaker(self, speaker_id):
        ...
        presentation_materials = (
            db.session.execute(
                db.select(PresentationMaterial).filter_by(
                    session_id=self.id, speaker_id=speaker.id
                )
            )
            .scalars()
            .all()
