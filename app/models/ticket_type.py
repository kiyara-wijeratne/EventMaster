...
class TicketType(db.Model):
    ...
    def get_total_tickets_quantity(cls, event_id, exclude_ticket_id=None):
        ...
        return sum(
            ticket.max_quantity for ticket in total_tickets if ticket.max_quantity
        )
    ...
    def create(cls, event_id, name, max_quantity, sales_start, sales_end, price):
        ...
        if ticket_type_sum + max_quantity > event.capacity:
            raise ValueError(
                f"Cannot allocate {max_quantity} tickets."
    ...
    def update(
        self, name=None, max_quantity=None, sales_start=None, sales_end=None, price=None
    ):
        from app.models.event import Event
        ...
        if max_quantity:
            ...
            if ticket_type_sum + max_quantity > event.capacity:
                raise ValueError(
                    f"Cannot alloccate {max_quantity} tickets."
                    f"The event only has capacity for {event.capacity - ticket_type_sum} tickets remaining."
                )
    ...
    def delete(self):
        ...
        if active_registrations:
            raise ValueError(
                f"Cannot delete {self.name} ticket."
