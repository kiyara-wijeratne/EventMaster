...
class Event(db.Model):
    ...
    branding: Mapped[Optional["EventBranding"]] = relationship(
    ...
    @classmethod
    def create(
        cls, title, event_type_id, venue, capacity, event_start, event_end, organiser_id
    ):
    ...
    def update(self, title=None, venue=None, capacity=None):
        ...
        if capacity:
            ...
            tickets = (
                db.session.execute(db.select(TicketType).filter_by(event_id=self.id))
                .scalars()
                .all()
            )
            # calculate the total number of ticket types allocated to an event
            tickets_quantity_sum = sum(
                ticket.max_quantity for ticket in tickets if ticket.max_quantity
            )
    ...
    def reschedule(self, new_start, new_end):
        ...
        conflicting_sessions = [
            ...
            if session.session_start < new_start or session.session_end > new_end
        ]

        if conflicting_sessions:
            names = ",".join(conflicting_sessions)
            raise ValueError(
                f"Cannot reschedule. The following sessions fall outside the new event schedule: {names}."
