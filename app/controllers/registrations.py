...
def attendee_list():
    ...
    approved_attendees = Registration.get_attendees(
        "Approved", filter_event_id=selected_event_id
    )
    ...
    if request.method == "POST":
        ...
        if registration:
            ...
            if not registration.check_in:
                CheckIn.create(
                    registration_id=registration_id, coordinator_id=current_user.id
                )

                flash(
                    f"Successfully checked in {registration.attendee.full_name}",
                    "success",
                )
            else:
                flash(
                    f"{registration.attendee.full_name} is already checked in", "error"
                )
        else:
        ...
        return redirect(
            url_for("registrations.check_in_queue", event_id=selected_event_id)
        )
...
def pending_approval():
    if request.method == "POST":
        ...
        if registration:
            try:
                ...
                flash(
                    f"Successfully approved {registration.attendee.full_name}",
                    "success",
                )
            ...
            return redirect(
                url_for("registrations.pending_approval", event_id=selected_event_id)
            )
    ...
    # get events to populate dropdown
    all_events = Event.query.order_by(Event.event_start.desc()).all()
    waitlisted_attendees = Registration.get_attendees(
        "Waitlisted", filter_event_id=selected_event_id
    )
