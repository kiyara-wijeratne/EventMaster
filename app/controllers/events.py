...
def manage_event():
    ...
    if request.method == "POST":
        try:
            ...
            return redirect(
                url_for("events.manage_event", event_id=event.id if event else None)
            )
    ...
    if speaker_id:
        ...
        if file_path:
            PresentationMaterial.create(
                speaker_id=speaker_id, session_id=session.id, file_path=file_path
            )
...
def update_session():
    ...
    if session:
        ...
        session.update(title=request.form["title"], room=request.form["room"])

        session.reschedule(
            datetime.fromisoformat(request.form["session_start"]),
